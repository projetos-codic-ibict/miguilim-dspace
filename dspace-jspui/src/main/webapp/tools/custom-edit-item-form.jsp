...
<dspace:layout style="submission" titlekey="jsp.tools.edit-item-form.title"
    ...
    %>
    <form id="edit_metadata" name="edit_metadata" method="post"
        ...
        <c:forEach items="<%= fieldInputFormList %>" var="fieldInputForm" varStatus="loop">
            <%
                FieldInputForm currentField = ((FieldInputForm) pageContext.getAttribute("fieldInputForm"));
                MetadataValue metadata =  fieldInputFormUtils.getFieldFromMetadataByKeys(currentField.getSchema(), currentField.getElement(), currentField.getQualifier());
            %>
            <c:choose>
                <c:when test="${fieldInputForm.simpleInputType != null && fieldInputForm.simpleInputType.equals('onebox')}">
                    <div class="form-group">
                        <label for="${loop.index}"> ${fieldInputForm.label} </label>
                        <input class="form-control" id="${loop.index}" type="text"
                        value="<%= metadata != null ? metadata.getValue().trim() : "" %>"
                        />
                    </div>
                </c:when>
                <c:otherwise>
                    <div class="form-group">
                        <label for="${loop.index}">${fieldInputForm.label}</label>
                        <textarea id="${loop.index}" class="form-control" rows="3">
                            <%= metadata != null ? metadata.getValue().trim() : "" %>
                        </textarea>
                    </div>
                </c:otherwise>
            </c:choose>
        ...
        <div class="table-responsive">
            <table class="table" summary="Edit item withdrawn table">
                <tr>
                        <%-- <th class="oddYYY¥¥RowEvenCol"><strong>Language</strong></th> --%>
                        <%-- <th class="oddYYY¥¥RowEvenCol"><strong>Language</strong></th> --%>
